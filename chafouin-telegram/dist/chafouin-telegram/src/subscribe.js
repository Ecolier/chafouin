...
export const subscribeScene = new Scenes.BaseScene(subscribeSceneToken);
const CHAFOUIN_BASE_URL = 'http://localhost:8080';
subscribeScene.enter(ctx => {
    const { inbound, outbound, date } = ctx.scene.state;
...
subscribeScene.action('@search', (ctx) => {
    ...
    source.addEventListener('update', function (e) {
        ...
        if (!((_a = ctx.chat) === null || _a === void 0 ? void 0 : _a.id)) {
            source.close();
            return;
        }
...
subscribeScene.action(/@save(?:\((.*)\))?/, ctx => {
    ...
    let data = {};
    try {
        data = JSON.parse((_a = ctx.match[1]) !== null && _a !== void 0 ? _a : {});
    }
    catch (e) {
        throw 'Cannot save malformed JSON object';
    }
    ctx.scene.state = Object.assign(Object.assign({}, ctx.scene.state), data);
    ctx.scene.reenter();
});
const paginate = (array, count, page) => {
...
const paginateTrips = (array, count, page, { action, key, select }) => {
...
subscribeScene.action(/@selectOutbound(?:\(([0-9])\))?/g, ctx => {
    var _a;
    const paginatedTrips = paginateTrips(availableStations, 5, parseInt((_a = ctx.match[1]) !== null && _a !== void 0 ? _a : '0'), {
...
subscribeScene.action(/@selectInbound(?:\(([0-9])\))?/g, ctx => {
    var _a;
    ctx.editMessageReplyMarkup({
        inline_keyboard: paginateTrips(availableStations, 5, parseInt((_a = ctx.match[1]) !== null && _a !== void 0 ? _a : '0'), {
