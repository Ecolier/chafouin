...
export const validateTripRequest = (validStations) => (req, res, next) => {
    const { outbound: outboundStation, inbound: inboundStation, date: departureDate, } = req.query;
    ...
    if (!validStations.find(stationName => stationName.toUpperCase() === outboundStation.toUpperCase())) {
        winston.info('Search query validation failed: unknown outbound station.');
        return res.status(400).send('Unknown outbound station');
    }
    if (!validStations.find(stationName => stationName.toUpperCase() === inboundStation.toUpperCase())) {
