...
export function subscribe(
  ...
  const source = new EventSource(
    encodeURI(
      `${scraperBaseUrl}/subscribe` +
        `?outbound=${outboundStation}` +
        `&inbound=${inboundStation}` +
        `&date=${departureDate}` +
        `&seats=true${filters.type ? `&type=${filters.type}` : ''}${
  ...
  source.addEventListener('channel', async (event) => {
    ...
    logger.info(
      `User ${userId} subscribed to ${schedule.toPath()} on channel no. ${channel}`
    );
  ...
  return {
    onUpdate(tripUpdateFn: (trips: Trips) => void) {
      source.addEventListener('update', (event) => {
        logger.info(
          `User ${userId} received trip update for ${schedule.toPath()}`
        );
...
export async function unsubscribe(userId: number, path: string) {
  ...
  logger.info(
    `User ${userId} unsubscribed from ${path} on channel no. ${alert.channel}`
  );
  fetch(
    encodeURI(
      `${scraperBaseUrl}/unsubscribe` +
...
export async function restore(
  restoreFunc: (userId: string, trips: Trips) => void
) {
  for await (const key of redis.scanIterator({
    ...
    Object.values(user.alerts).forEach(async (alert) => {
      ...
      logger.info(
        `Restored alert for user ${userId} for ` +
          `${schedule.toPath()} on channel ${alert.channel}`
      );
      return subscribe(userId, schedule, alert.filters, alert.channel).onUpdate(
        (trips: Trips) => restoreFunc(userId, trips)
      );
    });
  }
}
