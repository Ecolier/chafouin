...
const availableStations = Object.values(railwaysProvider.stations);
...
scene.enter((ctx) => {
  ...
  if (state.date) {
    message += `📆 On the *${new Date(state.date).toLocaleDateString(
      'fr-FR'
    )}*\n`;
  }
  const keyboard = [
    ...
    [
      {
        text:
          state.type === 'slow'
            ...
            ? '🚅 Fast trains only'
            : '💺 All trains',
        callback_data: `@save(${JSON.stringify({
          type:
            state.type === 'all'
              ? 'fast'
              : state.type === 'fast'
  ...
  if (
    Object.keys(ctx.scene.state).length !== 0 &&
...
scene.action('@search', async (ctx) => {
  ...
  if (alerts && alerts[schedule.toPath()]) {
    winston.info(
      `User ${userId} is already subscribed to ${schedule.toPath()}`
    );
    ...
    subscribe(userId, schedule, { type: state.type }).onUpdate((trips) =>
...
scene.action(/@selectOutbound(?:\(([0-9])\))?/g, (ctx) => {
  ctx.editMessageReplyMarkup({
    inline_keyboard: paginate.inlineKeyboard(
      availableStations,
      5,
